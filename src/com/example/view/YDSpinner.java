...
@SuppressLint("NewApi")
public class YDSpinner extends android.widget.Spinner{
	private boolean isMarquee=false;
	private static final String TAG = "TextView";
	private Context context;
	public YDSpinner(Context context, AttributeSet attrs) {
		super(context);
	...
	public void setAttributeSet(AttributeSet attrs){		
		...
		for(int i=0;i<count ;i++){
			...
			case popupBackground:
				String pbString=attrs.getAttributeValue(i);
				//显示颜色背景
				if(pbString.startsWith("@color/")||pbString.startsWith("#")){
				    this.setBackgroundColor(YDResource.getInstance().getIntColor(pbString));
				}else if(pbString.startsWith("@drawable/")){
					//颜色drawable背景
					this.setBackgroundDrawable(DrawableUtils.getDrawable(context, pbString,"res"));
				}
				break;
			default:
				break;
